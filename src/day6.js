...
const parseLine = line => {
  const splitLine = line.replace(/^turn /, 'turn').split(' ');
  const [verb, start, _, end] = splitLine;
...
const applyLine = line => {
  ...
  for (let row = result.startX; row <= result.endX; row++) {
    for (let col = result.startY; col <= result.endY; col++) {
      result.func(row, col);
    }
  }
};
