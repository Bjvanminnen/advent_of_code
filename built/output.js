...
/***/ function(module, exports, __webpack_require__) {
	...
	var countLights = function countLights() {
	  return lights.filter(function (item) {
	    return item;
	  }).length;
	};
	
	var totalBrightness = function totalBrightness() {
	  return lights.reduce(function (prev, cur) {
	    return prev + cur;
	  }, 0);
	};
	
	var getIndex = function getIndex(row, col) {
	  return row * 1000 + col;
	};
	
	var lightToggle = function lightToggle(row, col) {
	  lights[getIndex(row, col)] = lights[getIndex(row, col)] + 2;
	};
	
	var lightOn = function lightOn(row, col) {
	  lights[getIndex(row, col)] = lights[getIndex(row, col)] + 1;
	};
	
	var lightOff = function lightOff(row, col) {
	...
	var parseLine = function parseLine(line) {
	  var splitLine = line.replace(/^turn /, "turn").split(" ");
	
	  var _splitLine = _slicedToArray(splitLine, 4);
	
	  var verb = _splitLine[0];
	  var start = _splitLine[1];
	  var _ = _splitLine[2];
	  var end = _splitLine[3];
	
	  var _start$split = start.split(",");
	
	  var _start$split2 = _slicedToArray(_start$split, 2);
	
	  var startX = _start$split2[0];
	  var startY = _start$split2[1];
	
	  var _end$split = end.split(",");
	
	  var _end$split2 = _slicedToArray(_end$split, 2);
	
	  var endX = _end$split2[0];
	  var endY = _end$split2[1];
	
	  return {
	    func: verbToFunc[verb],
	...
	var applyLine = function applyLine(line) {
	  var result = parseLine(line);
	
	  for (var row = result.startX; row <= result.endX; row++) {
	    for (var col = result.startY; col <= result.endY; col++) {
	      result.func(row, col);
	    }
	  }
	};
	
	DATA.forEach(applyLine);
