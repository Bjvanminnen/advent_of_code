...
/***/ function(module, exports, __webpack_require__) {
	...
	var TEST_DATA = ["123 -> x", "456 -> y", "x AND y -> d", "x OR y -> e", "123 LSHIFT 2 -> f", "y RSHIFT 2 -> g", "NOT x -> h", "NOT y -> i"];
	
	var graph = {};
	
	var methods = {
	  AND: function AND(op1, op2) {
	    return op1 & op2;
	  },
	  OR: function OR(op1, op2) {
	    return op1 | op2;
	  },
	  LSHIFT: function LSHIFT(op1, op2) {
	    return op1 << op2;
	  },
	  RSHIFT: function RSHIFT(op1, op2) {
	    return op1 >> op2;
	  },
	  NOT: function NOT(op1, op2) {
	    return ~op2 + 65535 + 1;
	  }
	};
	
	var parseLine = function parseLine(line) {
	  line = line.replace(/NOT/g, "op1 NOT");
	
	  var _line$split = line.split("->");
	
	  var _line$split2 = _slicedToArray(_line$split, 2);
	
	  var left = _line$split2[0];
	  var target = _line$split2[1];
	
	  if (left.split(" ").length === 2) {
	    left = "0 OR " + left;
	  }
	
	  var _left$split = left.split(" ");
	
	  var _left$split2 = _slicedToArray(_left$split, 3);
	
	  var op1 = _left$split2[0];
	  var method = _left$split2[1];
	  var op2 = _left$split2[2];
	...
	var process = function process(val) {
	  if (curried[val] !== undefined) {
	    return curried[val];
	  }
	  ...
	  if (!graph[val]) {
	    ...
	    var _graph$val = graph[val];
	    var op1 = _graph$val.op1;
	    var method = _graph$val.method;
	    var op2 = _graph$val.op2;
	
	    var op1Val = process(op1);
	    var op2Val = process(op2);
	    result = methods[method](op1Val, op2Val);
	  }
	  curried[val] = result;
	  return result;
	};
	
	// TEST_DATA.forEach(parseLine);
